...
| `kubectl-pods-monitoring.png` | Terminal output showing monitoring namespace pods for Prometheus, Grafana, and related services | Capture the requested terminal output, export it as PNG, and replace `screenshots/kubectl-pods-monitoring.png` without changing the filename. |
...
| `helm-list.png` | Terminal output of Helm releases, such as `helm list -A` | Capture the requested terminal output, export it as PNG, and replace `screenshots/helm-list.png` without changing the filename. |
